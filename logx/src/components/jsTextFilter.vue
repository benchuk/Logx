...
<v-card>
    <v-container fluid class="pa-1">
        <v-layout row>
            ...
            <v-select v-if="editable" :items="types" v-model="filter.type" return-object item-text="name" label="Type" ></v-select>
        </v-layout>
...
export default {
    ...
    mounted(){
        ...
        if(this.filter){
            this.selectedType = this.types.find(x=>{ return x.name==this.filter.type})
